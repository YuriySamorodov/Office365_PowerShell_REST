...
Param (
    ...
    [string]$FilePath = ( Join-Path "$($env:USERPROFILE)\Desktop" -ChildPath ( ( Get-Date -Format "yyyy-MM-dd_HH-mm-ss" ) + '_' + 'V11-6-16' + '_' + 'MailStats' + '.csv' ) ),
    ...
    [string]$Log = ( Join-Path "$($env:USERPROFILE)\Desktop" -ChildPath ( ( Get-Date -Format yyyy-MM-dd ) + '_' + 'V11-6-16' + '_' + 'Log' + '_' + 'MailStats' + '.log' ) )
